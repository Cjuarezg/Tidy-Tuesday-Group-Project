...
plot <- ggplot(data = rating_genre)+
  ...
  geom_point(data=top_three, aes(x=genre, y=overall_rating, 
                                 color= genre, fill=genre,alpha=title), size=0.5)+
   geom_violin(aes(y = overall_rating, x=fct_infreq(genre), color=genre), size=0.3)+
...
plotly_json(plot)
ggplotly(plot) %>% 
  style(text="", hoverinfo = c('overall_rating','title'), traces = 56:106) 
...
nitys <- rating_genre %>% 
  filter(periode == "90's") %>% 
    ...
    color = ~genre,
    showlegend=FALSE,
    text= ~paste('Title: ', title, '</br> Rating:', overall_rating),
    points="all",
    hoverinfo='text'
    ) 
nitys <- nitys %>%
